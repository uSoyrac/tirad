...
def print_monthly_table(monthly: dict) -> None:
    ...
    if not months:
        return
    print(f"\n  {'─' * 52}")
    print(f"  {B}{CY}AYLIK P&L  (${CAPITAL:,.0f} başlangıç){R}")
    print(f"  {'─' * 52}")
    running = CAPITAL
    for m in months:
        pnl  = monthly[m]
        pct  = pnl / running * 100
        running += pnl
        col  = ok if pnl > 0 else (warn if abs(pnl) < 50 else bad)
        bar  = _pbar(min(abs(pnl), 600), 600, GR if pnl > 0 else RD, 8)
        print(f"  {m}  {col(f'{pnl:>+9,.0f}')}  {col(f'{pct:>+6.1f}%')}  {bar}")
    print(f"  {'─' * 52}")
    total  = sum(monthly.values())
    total_pct = total / CAPITAL * 100
    print(f"  {'TOPLAM':8}  {(ok if total>0 else bad)(f'{total:>+9,.0f}')}  "
...
""")
    ...
    net_pnl_s  = f"${port['final_eq'] - CAPITAL:>+,.0f}"
    print(f"  {'Başlangıç':<26} ${CAPITAL:>10,.0f}")
