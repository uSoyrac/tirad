...
def backtest_symbol_v2(symbol: str, df_full: pd.DataFrame, max_leverage_limit: int = 5) -> dict:
    """
    ...
    for i in range(WARMUP, len(df_full) - 1):
        ...
        # ── Leverage ve Liquidation Hesabı ────────────────────────
        if comp >= 8.0:
            max_lev_by_score = 5
        elif comp >= 6.5:
            max_lev_by_score = 4
        ...
        req_lev = math.ceil(RISK_PCT / sl_dist)
...
def main() -> None:
    head(f"GELİŞMİŞ ÇOKLU TF VE KALDIRAÇ MATRİS BACKTESTİ  "
         f"{datetime.utcnow():%Y-%m-%d %H:%M UTC}")

    symbols = ["ETH/USDT", "SOL/USDT", "BTC/USDT"]
    # Matris tanımı: (TF, BARS, MAX_LEVERAGE)
    matrix = [
        ("1h", 4500, 2),
        ("4h", 1200, 2),
        ("1d", 400,  2),
    ...
    h2("VERİ İNDİRME VE HAZIRLIK")
    print()
    for sym in symbols:
        for tf, bars, _ in matrix:
            if tf in cache[sym]:
                continue
            sys.stdout.write(f"  ⬇  {sym:10} | {tf:3} ({bars:4} bar) ...")
            sys.stdout.flush()
            df = ohlcv(sym, tf, bars)
    ...
    h2("MATRİS BACKTEST HESAPLAMALARI")
    print()
    
    # Başlıkları yazdır
    print(f"  {'Sembol':10} | {'TF':3} | {'Kaldıraç':8} | {'İşlem':5} | {'WR':6} | {'PF':5} | {'MaxDD':6} | {'Avg R':6} | {'Bitiş':8} | {'Likidasyon':11}")
    print(f"  {'-'*100}")

    for sym in symbols:
        for tf, bars, max_lev in matrix:
            df = cache[sym].get(tf)
            if df is None or df.empty:
                continue
            
            res = backtest_symbol_v2(sym, df, max_leverage_limit=max_lev)
            sum_ = res["summary"]
            ...
            pf = sum_["pf"]
            dd = sum_["max_dd"]
            avg_r = sum_.get("avg_r", 0)
            final_eq = sum_["final_eq"]
            
            liq_count = sum(1 for t in res["trades"] if t["liquidated"])
            levs = [t["leverage"] for t in res["trades"]]
            avg_lev = np.mean(levs) if levs else 1.0
            ...
            pc = ok if pf >= 1.5 else (warn if pf >= 1.0 else bad)
            dc = ok if dd <= 15 else (warn if dd <= 25 else bad)
            rc = ok if final_eq > 100 else bad
            l_col = ok if liq_count == 0 else bad
            
            lev_str = f"Maks {max_lev}x"
            print(f"  {B}{sym:10}{R} | {tf:3} | {lev_str:8} | {n:>5} | {wc(f'{wr:.1%}'):6} | {pc(f'{pf:.2f}'):5} | {dc(f'{dd:.1f}%'):6} | {avg_r:>+5.2f}R | {rc(f'${final_eq:.2f}'):8} | {l_col(str(liq_count)):11}")

    print("\n\n### DETAYLI MATRİS SONUÇLARI (MARKDOWN)\n")
    print("| Sembol | Zaman Dilimi | Maks Kaldıraç | İşlem Sayısı | Kazanma Oranı (WR) | Profit Factor (PF) | Max Drawdown (DD) | Ortalama R | Bitiş Değeri ($100 ile) | Likidasyon Sayısı |")
    print("|--------|--------------|---------------|--------------|--------------------|--------------------|-------------------|------------|-------------------------|-------------------|")
    for r in results_data:
        sym = r["symbol"].replace("/USDT", "")
        print(f"| **{sym}** | {r['tf']} | {r['max_lev']}x | {r['n']} | {r['wr']:.1%} | {r['pf']:.2f} | {r['max_dd']:.1f}% | {r['avg_r']:+.2f}R | **${r['final_eq']:.2f}** | {r['liq_count']} |")
    print()
