...
def backtest_symbol_v2(symbol: str, df_full: pd.DataFrame) -> dict:
    ...
    for i in range(WARMUP, len(df_full) - 1):
        ...
        lev = max(1, min(req_lev, max_lev_by_score))
        liq_dist = 1.0 / lev
...
""")
    ...
    # Ortalama kaldıraç ve likidasyon kontrolü
    total_trades = port["trades"]
    if total_trades:
        levs = [t["leverage"] for t in total_trades]
        avg_lev = np.mean(levs)
        max_lev = np.max(levs)
        max_sl_pct = np.max([t["sl_pct"] for t in total_trades])
        liq_count = sum(1 for t in total_trades if t["liquidated"])
        
        h2("KALDIRAÇ VE LİKİDASYON ANALİZİ")
        print()
        print(f"  {'Ortalama Kaldıraç':<26} {avg_lev:.2f}x")
        print(f"  {'Maksimum Kaldıraç':<26} {max_lev}x  (5x score limit)")
        print(f"  {'Maksimum Stop-Loss':<26} {max_sl_pct:.2f}%")
        print(f"  {'Likidasyon Sayısı':<26} {bad(str(liq_count)) if liq_count > 0 else ok(str(liq_count))}")
        print()
        if liq_count == 0:
            print(f"  {ok('✅ MATEMATİKSEL LİKİDASYON KORUMASI GÜVENLİ: Sıfır Likidasyon!')}")
            print(f"     Açıklama: 5x kaldıraçta likidasyon için fiyatın %20 ters hareket etmesi gerekir.")
            print(f"     Stratejimizin stop-loss mesafesi maksimum %10 ile sınırlıdır (bu backtestte maks %{max_sl_pct:.1f}).")
            print(f"     Stop-loss emirleri her zaman likidasyon fiyatından önce tetiklenir.")
        else:
            print(f"  {bad('⚠️ UYARI: Likidasyon gerçekleşti!')}")
        print()
