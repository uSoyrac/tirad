...
_WEIGHT_OI       = 0.11
_WEIGHT_FR       = 0.11
_WEIGHT_SESSION  = 0.09
_WEIGHT_VWAP     = 0.07
_WEIGHT_WYCKOFF  = 0.04
_WEIGHT_LIQ      = 0.07   # Liquidation: short/long tasfiye baskısı
_WEIGHT_REGIME   = 0.08   # Piyasa rejimi (ADX + volatilite)
_WEIGHT_OPTIONS  = 0.07   # Deribit opsiyon skew + DVOL
...
class ConfluenceScorer:
    ...
    def score(
        ...
        try:
            return self._compute(df, ms, oi_series, px_series, funding_rate, symbol)
    ...
    def _compute(
        ...
        # ── 9. CoinGlass Liquidation ──────────────────────────────
        liq_score = 0.0
        liq_res = {}
        if symbol:
            try:
                from market.coinglass_fetcher import fetch_coinglass_data
                cg = fetch_coinglass_data(symbol)
                if cg.get("available"):
                    raw_liq = cg.get("liq_score", 0.0)
                    # Yön uyumu: short tasfiyesi bullish için pozitif, tersine negatif
                    if ms.trend == Trend.BULLISH:
                        liq_score = raw_liq   # short squeeze = bullish için iyi
                    elif ms.trend == Trend.BEARISH:
                        liq_score = -raw_liq  # long tasfiyesi = bearish için iyi
                    liq_res = cg.get("liquidation", {})
                    details["liquidation"] = cg
            except Exception as _le:
                logger.debug("CoinGlass liquidation atlandı: %s", _le)
        liq_score = self._clamp(liq_score)

        # ── 10. Piyasa Rejimi (ADX + Volatilite) ─────────────────
        regime_score   = 0.0
        regime_blocked = False
        try:
            from analysis.regime_filter import detect_regime
            regime_res    = detect_regime(df)
            regime_score  = float(regime_res.get("regime_score", 0.0))
            regime_blocked = not regime_res.get("should_trade", True)
            details["regime"] = regime_res
        except Exception as _re:
            logger.debug("Rejim filtresi atlandı: %s", _re)
        regime_score = self._clamp(regime_score)

        # ── 11. Deribit Opsiyon Skew + DVOL ──────────────────────
        options_score = 0.0
        if symbol and not symbol.endswith(".IS"):
            try:
                from market.deribit_fetcher import fetch_deribit_score
                opt_res = fetch_deribit_score(symbol)
                if opt_res.get("available"):
                    raw_opt = opt_res.get("options_score", 0.0)
                    # Yön uyumu: yüksek korku (pozitif skew) bullish için iyi
                    if ms.trend == Trend.BULLISH:
                        options_score = raw_opt
                    elif ms.trend == Trend.BEARISH:
                        options_score = -raw_opt
                    details["options"] = opt_res
            except Exception as _oe:
                logger.debug("Deribit opsiyon atlandı: %s", _oe)
        options_score = self._clamp(options_score)
        ...
        weighted = (
            ...
            _norm(liq_score)     * _WEIGHT_LIQ      +
            _norm(regime_score)  * _WEIGHT_REGIME   +
            _norm(options_score) * _WEIGHT_OPTIONS
        )
