...
class MarketStructureAnalyzer(BaseAnalyzer):
    ...
    def _run_analysis(self, df: pd.DataFrame) -> MarketStructure:
        ...
        # ── 8. smc_lib cross-validation ───────────────────────────
        smc_lib_result = {}
        try:
            from analysis.smc_lib import full_smc_lib_analysis
            smc_lib_result = full_smc_lib_analysis(
                df,
                trend           = trend_str,
                existing_bos_bull = ms_raw.get("bos_bull", False),
                existing_bos_bear = ms_raw.get("bos_bear", False),
                existing_fvg_bull = bull_fvg,
                existing_fvg_bear = bear_fvg,
                existing_ob_bull  = bull_obs,
                existing_ob_bear  = bear_obs,
            )
            boost = smc_lib_result.get("cross_val", {}).get("score_boost", 0.0)
            score = min(10.0, round(score + boost, 2))

            # OTE smc_lib'den geliyorsa mevcut ote'yi zenginleştir
            lib_ote = smc_lib_result.get("ote", {})
            if lib_ote.get("in_ote") and not (ote and (ote.get("bull_ote") or ote.get("bear_ote"))):
                ote = ote or {}
            ...
            logger.debug("smc_lib cross-validation atlandı: %s", _e)
