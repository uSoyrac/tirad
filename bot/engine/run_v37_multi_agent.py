...
def run_v37_multi_agent():
    ...
    print("Ajan 1 Eğitiliyor: Gözcü (The Watcher - Chop Filtresi)...")
    ...
    print("Ajan 2 Eğitiliyor: Keskin Nişancı (The Sniper - Kırılım Yönü)...")
    ...
    print("Özel Tim Eğitimi Tamamlandı.")
    
    balance = START_BALANCE
    open_positions = []
    trade_history = []
    
    print("\nSaf Kör Test (V37 Multi-Agent OOS) Başlıyor. Başlangıç Kasası: $100")
    ...
    for ts, group in grouped:
        ...
        for pos in open_positions:
            ...
            # Ajan 3: Risk Yöneticisi (İz Süren Stop ve Çıkış)
            ...
            if low <= pos['sl_price']:
                ...
                # Maksimum bekleme süresi (15 gün)
                if (ts - pos['entry_time']).total_seconds() / 3600 >= 360:
        ...
        if len(group) > 0:
            ...
            for i, row in group.reset_index(drop=True).iterrows():
                ...
                # Ajan 1 Onayı (Gözcü): Piyasa Chop Değil! Trend Bekleniyor (> %65 İhtimal)
                if prob_trend > 0.65:
                    # Ajan 2 Onayı (Nişancı): Temiz Kırılım (Breakout) Var! (> %60 İhtimal)
                    if prob_sniper > 0.60:
                        candidates.append((sym, row, prob_sniper))
                        
            if candidates:
                # Birden fazla fırsat varsa en güçlü kırılımı (Sniper Prob) seç
                ...
                # Ajan 3: Risk Yöneticisi (The Manager)
                # Kelly (Max %20)
    ...
    print("\nSimülasyon Tamamlandı. V37 Multi-Agent Raporu Oluşturuluyor...\n")
    
    if len(trades_df) == 0:
         print("Sistem hiç işlem açmadı! Ajanlar her anı Chop olarak değerlendirdi.")
         return
    ...
    print("\n--- V37 MULTI-AGENT (ÖZEL TİM) ÖZETİ (1 YIL KÖR TEST) ---")
