...
def fetch_oi_and_funding(symbol: str, limit: int = 10) -> tuple:
    ...
    Öncelik:
      1. Binance fapi public endpoint (openInterestHist) — gerçek USD bazlı seri
      2. ccxt fetch_open_interest_history — model bazlı fallback
    API key gerekmez.
    """
    ...
    # ── Funding Rate (ccxt en güvenilir) ──────────────────────────
    try:
        import ccxt as _ccxt
        _exc = _ccxt.binance({"enableRateLimit": True, "options": {"defaultType": "future"}})
        fr_data = _exc.fetch_funding_rate(symbol)
        funding = fr_data.get("fundingRate")
    except Exception:
        pass

    # ── OI Geçmişi: Binance public fapi endpoint ──────────────────
    try:
        r = _req.get(
            "https://fapi.binance.com/futures/data/openInterestHist",
            params={"symbol": sym_clean, "period": "4h", "limit": limit},
            timeout=10,
        )
        if r.status_code == 200:
            ...
            if isinstance(data, list) and data:
                ...
                # Eşleşen kapanış fiyatları: aynı endpoint'teki kline verisi
                ts_list = [int(d.get("timestamp", 0)) for d in data]
                klines_r = _req.get(
                    "https://fapi.binance.com/fapi/v1/klines",
                    params={"symbol": sym_clean, "interval": "4h", "limit": limit + 2},
                    timeout=10,
                )
                if klines_r.status_code == 200:
                    klines = klines_r.json()
                    # kline[0]=open_time, kline[4]=close
                    kline_map = {int(k[0]): float(k[4]) for k in klines}
                    px_list = []
                    for ts in ts_list:
                        # OI timestamp ile en yakın kline eşleştir (±30 sn tolerans)
                        closest = min(kline_map.keys(), key=lambda t: abs(t - ts), default=None)
                        px_list.append(kline_map.get(closest, 0.0) if closest else 0.0)
                else:
                    px_list = [0.0] * len(oi_list)

                if all(p == 0.0 for p in px_list):
                    px_list = None   # Fiyat verisi gelmedi, fallback tetikle

    except Exception as e:
        logger.debug("Binance fapi OI endpoint hatası (%s): %s", symbol, e)
    ...
    if oi_list is None:
        try:
            import ccxt as _ccxt
            _exc = _ccxt.binance({"enableRateLimit": True, "options": {"defaultType": "future"}})
            if hasattr(_exc, "fetch_open_interest_history"):
                oi_hist = _exc.fetch_open_interest_history(symbol, "4h", limit=limit)
                if oi_hist:
                    oi_list = [float(x.get("openInterest", 0)) for x in oi_hist]
                    px_list = [float(x.get("close", 0)) for x in oi_hist]
        except Exception as e:
            logger.debug("ccxt OI fallback hatası (%s): %s", symbol, e)
