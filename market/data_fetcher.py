...
class BinanceFetcher:
    ...
    def fetch_oi_history(self, symbol: str, interval: str = "4h", limit: int = 20):
        """
        Binance Futures Open Interest geçmişi — public endpoint, API key gerekmez.
        interval: "5m" | "15m" | "30m" | "1h" | "2h" | "4h" | "6h" | "12h" | "1d"
        Returns: (oi_value_list, close_price_list) — her iki liste eşleşik timestamp'e sahip
        """
        sym = symbol.replace("/", "")
        try:
            # OI zaman serisi
            ...
            oi_list = [float(d.get("sumOpenInterestValue", 0)) for d in data]
            ts_list = [int(d.get("timestamp", 0)) for d in data]

            # Eşleşen kline kapanış fiyatları
            kl = requests.get(
                "https://fapi.binance.com/fapi/v1/klines",
                params={"symbol": sym, "interval": interval, "limit": limit + 2},
                timeout=10,
            )
            px_list = [0.0] * len(oi_list)
            if kl.status_code == 200:
                klines = kl.json()
                # kline formatı: [open_time, open, high, low, close, ...]
                kline_map = {int(k[0]): float(k[4]) for k in klines}
                for i, ts in enumerate(ts_list):
                    closest = min(kline_map.keys(), key=lambda t: abs(t - ts), default=None)
                    if closest is not None:
                        px_list[i] = kline_map[closest]

            return oi_list, px_list
        except Exception as e:
            logger.debug("OI history fetch error %s: %s", symbol, e)
            return None, None

    def fetch_long_short_ratio(self, symbol: str, interval: str = "4h", limit: int = 10):
        """
        Global Long/Short hesap oranı — public endpoint.
        Returns: list of float (son değerden eskiye)
        """
        sym = symbol.replace("/", "")
        ...
        try:
            ...
            if not isinstance(data, list):
                return []
            return [float(d.get("longShortRatio", 1.0)) for d in data]
        except Exception as e:
            logger.debug("L/S ratio fetch error %s: %s", symbol, e)
            return []

    def fetch_taker_volume_ratio(self, symbol: str, interval: str = "4h", limit: int = 10):
        """
        Taker buy/sell hacim oranı — public endpoint.
        > 1.0 → alıcılar baskın (bullish baskı)
        < 1.0 → satıcılar baskın (bearish baskı)
        ...
        try:
            ...
            if not isinstance(data, list):
                return []
            return [float(d.get("buySellRatio", 1.0)) for d in data]
        except Exception as e:
            logger.debug("Taker vol ratio fetch error %s: %s", symbol, e)
            return []
...
class MarketDataFetcher:
    ...
    def fetch_crypto_extras(self, symbol: str) -> dict:
        oi_hist, _ = self.binance.fetch_oi_history(symbol, interval="4h", limit=20)
        ls_ratio = self.binance.fetch_long_short_ratio(symbol, interval="4h", limit=10)
        taker_ratio = self.binance.fetch_taker_volume_ratio(symbol, interval="4h", limit=10)
        return {
