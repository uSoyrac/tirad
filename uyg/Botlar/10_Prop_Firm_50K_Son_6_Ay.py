#!/usr/bin/env python3
"""
HİBRİD MODEL PROP FIRM SİMÜLATÖRÜ (50K HESAP İÇİN)
- Hesap: $50K, Daily DD: 5%, Max DD: 10%
- Her işlemde %1 Risk alınır (-$500 Kayıp).
- Her kârlı işlemde +%2 Kâr alınır (+$1000 Kâr).
- Son 6 aylık periyot (Aralık 2025 - Mayıs 2026) test edilecek.
"""
...
def run_journey_50k(rows, P, gate_top=0.20, start_date_str="2025-12-01", payout_threshold=1000):
    ...
    for i, r in enumerate(rows):
        ...
        if current_month is None:
            ...
            print(f"    - Aşama : {phase} | Bakiye: ${eq:,.2f}")
            print(f"    - İşlem Sayısı: {month_trades} (Win Rate: %{win_rate:.1f})")
            print("-" * 70)
            current_month = trade_date.month
            month_trades = 0
            month_wins = 0
...
def main():
    ...
    thresholds = [1000, 2500] # 50K hesap için 1000$ veya 2500$ vur-kaç hedefleri
    start_dates = ["2025-12-01"] # Son 6 Ay
    
    print("="*85)
    print(" 🏙️  WALL STREET: 50K HİBRİD PROP FIRM (SON 6 AY SİMÜLASYONU) 🏙️")
    print("="*85)
    ...
    for start in start_dates:
        for th in thresholds:
            ...
            status, final_phase, total_payout, history, max_eq = run_journey_50k(
                r_smart, p_smart, gate_top=0.20, start_date_str=start, payout_threshold=th
            )
            
            print(f"\n📊 --- 50K HİBRİD KARİYER ÖZETİ ---")
