...
def main():
    print("="*80)
    print(" 🤖 V20 AI BACKTEST: XGBOOST YÖNETİMİNDE 2 YILLIK SİMÜLASYON")
    print("="*80)
    ...
    xgb_threshold = meta["threshold"]
    
    print(f"✅ XGBoost Modeli yüklendi. (Threshold: {xgb_threshold:.2f}, Beklenen WR: %{meta['expected_win_rate']*100:.1f})")
    ...
    for coin in COINS:
        ...
        for i in range(250, len(df) - 100):
            ...
            # V18 Base Filters (To prevent trash data entering XGBoost)
            if df["adx"].iloc[i-1] > 40 or df["vol_ratio"].iloc[i-1] > 3.0: continue
            
            # 🧠 AI TAHMİNİ (PREDICT_PROBA)
            features_dict = {
                "adx": [df["adx"].iloc[i-1]],
            ...
            # 🛑 THRESHOLD FİLTRESİ
            if win_prob < xgb_threshold: continue
    ...
    print("\n" + "="*80)
    print(f" 🏆 V20 AI ŞAMPİYONU: 2 YILLIK NİHAİ SONUÇ")
    print("="*80)
    ...
    with open("/Users/uygar/.gemini/antigravity/scratch/tirad/uyg/docs/v20_ai_report.md", "w") as f:
        f.write("# 🤖 V20 XGBoost AI Test Raporu (2 Yıllık)\n\n")
        f.write(f"**XGBoost Threshold:** {xgb_threshold:.2f}\n")
