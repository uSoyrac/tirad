...
"""
═══════════════════════════════════════════════════════════════════════════════
V71 HYBRID EXPO ENGINE — Dinamik Kelly Katlaması (Claude's Core + V71 Vision)
═══════════════════════════════════════════════════════════════════════════════

Bu sistem, Claude'un doğrulanmış 'compound_engine.py' altyapısını kullanır ancak
sabit "Notional" yerine kullanıcının "Dinamik Kelly" (Expo Büyüme) vizyonunu uygular.

YENİ MANTIK (Percentile Based Dynamic Kelly):
  Yapay zekanın olasılık çıktıları (P) yüzdelik dilimlere ayrılır:
  1. KUSURSUZ FIRSAT (Top %2 Sinyaller):  2.5x Notional (Claude'un Kelly Zirvesi)
  2. ALTIN FIRSAT    (Top %10 Sinyaller): 2.0x Notional 
  3. NORMAL FIRSAT   (Top %30 Sinyaller): 1.0x Notional

KURAL İHLALİ YOK: Claude'un ispatladığı "2.5x Notional Max" sınırı korunmuştur.
Bu sayede volatility drag'den korunarak, sadece en asimetrik işlemlerde gaza basılır.
═══════════════════════════════════════════════════════════════════════════════
"""
...
def main():
    ...
    print("\n[V71 HİBRİT EXPO MOTORU ÇALIŞTIRILIYOR...]")
    r=backtest_hybrid(rows, P, bankroll=250.0)
    
    print("="*72)
    print(f"  V71 EXPO BÜYÜME (OOS {OOS_START}→2026, $250 Kasa)")
    print("="*72)
    ...
    print(f"\n  SONUÇ: Maksimum 2.5x sınırını ihlal etmeden 'Dinamik Kelly' uygulanmıştır.")
