...
"""
═══════════════════════════════════════════════════════════════════════════════
OPTIMAL SCALP HARVEST BOT (Yüksek Frekanslı Kâr Çekimi)
═══════════════════════════════════════════════════════════════════════════════

Bu sistem, kullanıcının "Sürekli ve Sık Sık Kâr Alalım" talebi üzerine
tasarlanmış Yüksek Frekanslı Scalping (Vur-Kaç) botudur.

STRATEJİ:
- AI Modeli: Smart Money İvmesi (Yüksek isabetli yön tahmini)
- İşlem Tipi: Hızlı Scalping (DCA Kesinlikle Yok)
- Risk (Kaldıraç): Kasanın tamamı (1x) * 5x Kaldıraç = Toplam 5.0 Notional.
- Kâr Hedefi (TP): Varlık bazında %1.5 -> 5x Kaldıraçla Kasa bazında +%7.5 Kâr.
- Zarar Kes (SL): Varlık bazında %1.5 -> 5x Kaldıraçla Kasa bazında -%7.5 Zarar.
- Hasat Kuralı: Kasa $110'a ulaştığında anında 10 Dolar nakit çek, 100 Dolar
  ile ticarete sıfırdan devam et. 
═══════════════════════════════════════════════════════════════════════════════
"""
...
COINS      = ["BTC", "ETH", "SOL"]
TP, SL     = 0.015, 0.015         # +%1.5 TP / -%1.5 SL (Çok Dar Scalp Oranları)
HMAX       = 24                   # timeout = 1 gün
COST       = 0.0018               
OOS_START  = "2024-01-01"
GATE_TOP   = 0.20                 # En güvenilir %20 sinyal
LEVERAGE   = 5.0                  # 5x Kaldıraç
HARVEST_TARGET = 110.0            # 110 dolara ulaşınca 10 Dolar Nakit çek
...
def build_signals(cache="/tmp/scalp_harvest_sigs.pkl"):
...
def main():
    print(__doc__)
    print("Sinyaller hazırlanıyor (İlk çalıştırma birkaç dakika sürebilir, çünkü Hedefler değişti)...")
    rows=build_signals()
    print("Yapay Zeka Geçmiş Verilerle Eğitiliyor...")
    P=walk_forward_proba(rows)
    
    print("\n[ SCALP HASAT (SCALP HARVEST) BOTU ÇALIŞTIRILIYOR... ]")
    r=backtest_harvest(rows, P, starting_bankroll=100.0)
    
    print("==========================================================")
    print(f"  YÜKSEK FREKANSLI SCALP HASADI (OOS {OOS_START}→2026, $100 Kasa)")
    print("  Kural: 100 Dolar 110 Olunca Anında 10 Dolar Nakit Çek!")
    print(f"  Kaldıraç: {LEVERAGE}x  |  TP: %1.5  |  SL: %1.5")
    print("==========================================================")
    print(f"  Kazanma Oranı (Win Rate)              : %{r['wr']:.1f} ({r['n']} İşlem)")
    print(f"  Başarılı Hasat (110'a Ulaşma) Sayısı  : {r['harvest_count']} Kez")
