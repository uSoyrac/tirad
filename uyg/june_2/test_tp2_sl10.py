...
COINS      = ["BTC", "ETH", "SOL"]
TP, SL     = 0.02, 0.10          # +%2 TP / -%10 SL (Neredeyse Stopsuz)
HMAX       = 144                 # Hedefe ulaşması için daha uzun süre (6 gün)
COST       = 0.0018               
GATE_TOP   = 0.20                 
NOTIONAL   = 1.0                 # 1x Kaldıraç (Kaldıraçsız)
...
def build_signals(cache="/tmp/tp2_sl10_sigs.pkl"):
...
def backtest(rows, P, bankroll=100.0):
...
def main():
    ...
    r=backtest(rows, P, bankroll=100.0)
    
    print("="*72)
    print(f"  RAW ACCURACY TEST (OOS {OOS_START}→2026, $100 kasa)")
    print(f"  TP: %2  |  SL: %10  |  Kaldıraç: {NOTIONAL}x")
    print("="*72)
    print(f"  Kazanma Oranı (Win Rate): %{r['wr']:.1f} (Kazançlar: {r['wins']}, Kayıplar: {r['losses']})")
    print(f"  $100 → ${r['eq']:.0f}   |   CAGR %{r['cagr']:.1f}   |   MaxDD %{r['mdd']:.1f}   |   Toplam İşlem: {r['n']}")
